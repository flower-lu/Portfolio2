...
if __name__ == "__main__":
    ...
    iteration = 0
    history_best_scores = []  # 用于记录每次迭代后的历史最优得分
    history_best_params = []  # 用于记录每次迭代后的历史最优参数
    ...
    for params in all_params:
        ...
        history_best_params.append(best_params)
        print(f'Score: {score}')
        print(f'Iteration: {iteration}')
        print(f'Parameters: {params}')
        # 输出历史最优参数和得分
        print(f'History - Best Parameters: {history_best_params[-1]}, History - Best Score: {history_best_scores[-1]}')
        print('-' * 50)  # 打印长线条
        print()
